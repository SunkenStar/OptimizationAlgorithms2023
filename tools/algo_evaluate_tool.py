...
def algo_test_single(algorithm, name, function_index, margin=1e-3, verbose=False, silent=False):
    """
    ...
    for func in function_index:
        res = algorithm(func.function_body,
                        func.dimension,
                        func.zone)
...
def algo_test_multi(algorithm, name, function_index, times=50, margin=1e-3):
    """
    ...
    margin：允许的结果误差范围\n
    """
    print(f'开始对{name}的多轮测试')
    total_passes = 0
    total_tests = 0
    for _ in range(times):
        single_res = algo_test_single(algorithm, name, function_index, margin, False, True)
        total_passes += single_res[0]
        total_tests += single_res[1]
    print(f'进行{total_tests}轮测试，{name}通过{total_passes}轮，通过率{total_passes / total_tests * 100:.2f}%\n')
